...
例如，符号 $and$ 无法用来表示句子 “John and Bill are friends” 中的 and，因为在这个句子中，and 并不是用来连接两个陈述句。#footnote[译者注：中文中几乎不会出现这种情况]
符号 $and$ 和 $or$ 只能用于两个陈述句之间，表示它们的合取或析取；

符号 ¬ 只能放在一个陈述句前，用来否定该陈述句。
...
#exercises((1, 5))[
  ...
  + 下列哪些表达式是良构公式（well-formed formulas）： \
    ...
    (c) $P and not P$ \
    (d) $(P and Q)(P or R)$
  + 令 $P$ 表示陈述 “我将买裤子”，令 $S$ 表示陈述 “我将买衬衫”。下列表达式分别对应的英文句子是什么？ \
    (a) $not ( P and not S )$ \
    (b) $not P and not S$ \
    (c) $not P or not S$
  + 令 $S$ 表示陈述 “史蒂夫很开心”，令 $G$ 表示陈述 “乔治很开心”。下列表达式分别对应的英文句子是什么？ \
