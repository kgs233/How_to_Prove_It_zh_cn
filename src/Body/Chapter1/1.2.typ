...
$not P$ 的真值表也很容易构造，因为要使 $not P$ 为真，$P$ 必须为假。这个表见@table-1.2 。
...
#par()[]
关于 $P or Q$ 的真值表要稍微复杂一些。前三行显然应该如@table-1.3 所示填写，但对最后一行可能会有疑问。当 $P$ 和 $Q$ 都为真时，$P or Q$ 应该是真还是假？换句话说，$P or Q$ 是表示“$P$ 或 $Q$，或两者都真”，还是表示“$P$ 或 $Q$，但不能同时为真”？
...
第二种解释被称为_排斥或（Exclusive OR）_#footnote[译者注：排斥或的另外一个更加常见的名字叫做异或（XOR），符号为 $xor$]。在数学中，$or$ 总是表示包含或，除非另有特别说明，所以我们将 $or$ 理解为包含或。因此，我们如 @table-1.4 所示完成 $P or Q$ 的真值表。关于排斥或的更多内容，参见练习 3。

使用这些真值表中总结的规则，我们现在可以为更复杂的公式推导真值表。我们只需逐步计算公式各个组成部分的真值——从单个字母开始，逐步构造出更复杂公式的真值。

#example([为公式 $not (P or Q)$ 制作真值表。], label-name: "ex-1.2.1")[]
#solution[
  #align(center)[
    #table(
...
#example([为公式 $not (P and Q) or not R$ 制作真值表。], label-name: "ex-1.2.2")[]
#solution[
  #align(center)[
    #table(
...
这里有一种更简洁地制作真值表的方法。与其使用单独的列来列出公式各个组成部分的真值，不如直接把这些真值写在原公式中相应的连接词符号下面。@table-1.5 展示了这种方法，使用的公式来自例 1.2.1。
...
现在我们可以看到如何使用真值表来验证这个论证的有效性。@table-1.6 展示了该论证的两个前提和结论的真值表。回想一下，我们决定如果前提不可能全部为真而结论却为假的话，就称该论证为有效。
...
查看@table-1.6，我们发现表中唯一一行两个前提都为真的行是第三行，而在这一行中结论也为真。因此，真值表证实了如果前提都为真，结论也必须为真，所以这个论证是有效的。

#example([判断下列论证是否有效。], label-name: "ex-1.2.3")[
  + 要么约翰不愚蠢且懒惰，要么他很愚蠢。 \
    约翰很愚蠢。 \
    因此，约翰不懒惰。 \
  + 管家和厨师不可能都无辜。 \
...
#example([下面哪些公式是等价的？], label-name: "ex-1.2.4")[
  #align(center)[
...
#example([找出与下列公式等价的更简洁的公式：], label-name: "ex-1.2.5")[
  + $not (P or not Q).$
...
#example([这些命题是 永真式、矛盾式，还是 两者都不是？], label-name: "ex-1.2.6")[
  $
    not (P and Q),#h(3em)not P and not Q,#h(3em)not P or not Q.
  $
]
#solution[
  ...
  #h(2em)从真值表中可以清楚地看出，第一个公式是永真式，第二个是矛盾式，第三个既不是永真式也不是矛盾式。实际上，由于最后一列与第一列相同，第三个公式等价于 $P$。
]

我们现在可以陈述一些涉及永真式和矛盾式的更有用的定律。你应该能够通过思考相关命题的真值表会是什么样子，来自行确认这些定律都是正确的。
...
#solution[
  ...
  + $not (P or(Q and not R))and Q$
    $
            "等价于" & #h(1em) (not P and not (Q and not R)) and Q#h(1em)    && "（德摩根定律）"， \
...
#exercises((1, 5, 7, 9, 14, 16))[
  ...
  + 使用真值表判断这些命题哪些是永真式，哪些是矛盾式，哪些既不是永真式也不是矛盾式。 \
  ...
  + 假设一个论证的结论是永真式。你能得出关于该论证有效性的什么结论？
    如果结论是矛盾式呢？
    如果其中一个前提是永真式或矛盾式呢？
]
