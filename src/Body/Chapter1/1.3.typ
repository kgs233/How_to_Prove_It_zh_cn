...
#example([分析下列陈述的逻辑形式：], label-name: "ex-1.3.1")[
...
#example([将以下集合定义改写为元素性的形式：], label-name: "ex-1.3.2")[
  + $E = {2, 4,6, 8, dots}.$
...
#example([这些陈述意味着什么？每个陈述中的自由变量是什么？], label-name: "ex-1.3.3")[
  + $a + b in.not {x | x "是偶数"}$。
  + $y in {x | x "能被" w "整除"}$。
...
#definition([], label-name: "def-1.3.4")[
...
#example([以下陈述的真值集合是什么？], label-name: "ex-1.3.5")[
  + 莎士比亚写了 $x$。
  + $n$ 是一个偶数质数。
]
#solution[
