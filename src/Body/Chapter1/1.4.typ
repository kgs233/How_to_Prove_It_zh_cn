...
#definition([两个集合 $A$ 和 $B$ 的_交集（intersection）_是集合 $A inter B$，其定义如下：], label-name: "def-1.4.1")[
  $
...
#example([假设 $A = {1, 2, 3, 4, 5}$ 和 $B = {2, 4, 6, 8, 10}$。列出以下集合的元素：], label-name: "ex-1.4.2")[
  + $A inter B$.
  + $A union B$.
  + $A without B$.
...
#example([分析下列命题的逻辑形式：], label-name: "ex-1.4.4")[
...
$
      "等价于" #h(0.5em) & x in A and not (x in B and x in C)                 && #h(1em) "（例 1.4.4），" \
...
#definition([], label-name: "def-1.4.5")[
...
#v(-1em)
#example([], label-name: "ex-1.4.6")[
  假设 $A = {"red", "green"}, B = {"red", "yellow", "green", "purple"}, C = {"blue",$ $"purple"}$。
...
$
  ...
  "进一步等价于" x in A and (x in B and x in.not B)。
$
